...
| API/token 费用 | 查询失败 | 余额监控失败：读取到的余额字段异常（data.total_available=-664080523），未将其当作真实人民币余额；请配置正确的 MICU_API_BALANCE_JSON_PATH。 |
...
- 提醒线：50 元人民币。
- 当前状态：余额监控失败：读取到的余额字段异常（data.total_available=-664080523），未将其当作真实人民币余额；请配置正确的 MICU_API_BALANCE_JSON_PATH。
...
| 成本项 | 当前判断 | 说明 |
| --- | --- | --- |
| 米促 API/token | 查询失败 | 余额监控失败：读取到的余额字段异常（data.total_available=-664080523），未将其当作真实人民币余额；请配置正确的 MICU_API_BALANCE_JSON_PATH。 |

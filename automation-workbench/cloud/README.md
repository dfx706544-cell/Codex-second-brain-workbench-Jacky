...
- 当前状态：本工作区已绑定 GitHub 远程仓库；云端每日/每周工作流骨架、邮件发送器、API 余额提醒和公开模板发布流程已具备。仍需要以 GitHub Actions 实际运行记录确认长期稳定性
...
每日/每周云端任务需要同步人民币成本口径：

- LLM/API/token：读取真实账单或余额接口；无法读取时标注待核实。
- 米促 API：可核实余额低于 50 元人民币时提醒充值。
- GitHub Actions/Codex Cloud：免费额度内通常 0 元，超出以账单为准。
- 邮箱：163 SMTP 白名单邮件通常不另收发送费用，但要监控失败率和频率限制。
- 第三方平台：Kalodata、FastMoss、达秘/TikClubs、数据源和剪辑工具按各自套餐单独核算。

...
说明：
1. 这是给朋友复制框架和设计的公共模板，不包含 Jacky 的历史记录、outputs、队列、账号权限或后台自动化。
...
10. 开始执行时先给出本次任务预计人民币成本口径；无法精确核算时给出保守区间，并标注哪些费用待账单/API 核实。
11. 如果涉及 API 费用或 token 余额，请检查是否已配置真实账单/余额来源；当可核实余额低于 50 元人民币时提醒充值。无法读取真实余额时标注待授权，不要编造金额。
12. 如果涉及 Skill 安装，先评估候选和风险；即使用户开放权限，也必须针对具体候选等待确认后再安装。
13. 最后用简洁中文说明完成了什么、文件在哪里、预计/实际成本口径、来源链接有哪些、哪些草稿等待发送确认、还有哪些需要用户补充。

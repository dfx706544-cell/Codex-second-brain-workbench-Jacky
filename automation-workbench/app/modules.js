...
5. 开机后本地平台补采：如果任务涉及关机期间云端拿不到的账号后台数据，请在电脑开机、工作台可用、平台已登录后，读取 Kalodata、FastMoss、达秘 / TikClubs、TikTok、抖音、小红书、视频号、YouTube、Instagram、飞书、163 邮箱或剪映的已授权可见页面，或读取 inputs/ 导出的 CSV/Excel/截图说明，补齐作品数据、达人沟通、询盘、转化、变现、平台后台指标和来源链接；输出 boot-backfill 报告并更新知识库。
6. 关机后云端信息源只使用公开网页、RSS、AnySearch/搜索 API、金融市场数据 API、官方公告/财报、官方平台博客/帮助中心、公开趋势页、学术元数据 API、GitHub 数据、已授权邮箱 API 和飞书 API；不要声称能读取本机浏览器登录态或桌面软件。
7. 金融相关只做资讯、信号提醒、纸面交易、风险清单、持仓监控建议和人工确认前检查，不执行真实下单、支付或交易。
8. 邮件、微信、飞书、社交私信、上传、发布、提交、安装第三方 skill/plugin/software 等外部动作，先生成草稿、候选清单和确认清单；已配置白名单 SMTP 邮件可按安全发送器规则自动发给我的邮箱，其他真正外发或安装前等待我当次确认。
9. 开始执行时先给出本次任务预计人民币成本口径；无法精确核算时给出保守区间，并标注哪些费用待账单/API 核实。
10. 如果涉及 API 费用或 token 余额，请检查是否已配置真实账单/余额来源；当可核实余额低于 50 元人民币时提醒我充值。无法读取真实余额时，标注“余额监控未配置/待授权”，不要编造金额。
11. 最终把结果、报表、草稿、来源链接和任务记录保存到 outputs/，并尽量更新 automation-workbench/data/task-history.json、daily-briefs.json、business-feedback.json 或 knowledge-items.json。
12. 最后用简洁中文说明完成了什么、文件在哪里、预计/实际成本口径、来源链接有哪些、哪些动作等待我确认、哪些平台或权限还需要补齐。`,
  inbox: `请启动信息助手。
...
4. 关机后云端信息源限定为公开网页、RSS、AnySearch/搜索 API、金融市场数据 API、官方公告/财报、官方平台博客/帮助中心、公开趋势页、学术元数据 API、GitHub 数据、已授权邮箱 API 和飞书 API；本机登录态平台后台、夸克、剪映、微信、飞书客户端等只在开机后补采。
5. 开机后本地平台补采：电脑开机且工作台可运行时，读取已登录平台的可见后台、inputs/ 导出表格或用户提供截图，补齐云端缺失的账号数据、达人沟通、作品流量、转化、变现和平台后台来源，并写入知识库和业务反馈。
6. 结果尽量写入 outputs/ 和 automation-workbench/data/，并保留来源链接、时间戳和下一步动作。`,
